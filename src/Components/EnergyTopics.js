...
export default function EnergyTopics({ fetchedData }) {
  ...
  const makeGraph = () => {
    ...
    for (let i = 0; i < fetchedData.length; i++) {
      if (fetchedData[i].topic !== "" && fetchedData[i].sector === "Energy")
        topics.push(fetchedData[i].topic);
    }
    ...
    uniqueYears.forEach((year, i) => {
      for (let k = 0; k < fetchedData.length; k++) {
        if (
          fetchedData[k].topic === uniqueTopics[0] &&
          year === fetchedData[k].published.slice(-13, -9)
        ) {
          countTopic1[i]++;
        }
      }
    ...
    uniqueYears.forEach((year, i) => {
      for (let k = 0; k < fetchedData.length; k++) {
        if (
          fetchedData[k].topic === uniqueTopics[1] &&
          year === fetchedData[k].published.slice(-13, -9)
        ) {
          countTopic2[i]++;
        }
  ...
  return (
    <div
      ...
      className=" p-3 mb-5 bg-white  bg-dark"
    >
